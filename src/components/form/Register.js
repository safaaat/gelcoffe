...
function Register({
  ...
  const handleOnRegister = event => {
    ...
    } else if (!rqExp.test(dataValueRegister.email)) {
      clearValidEmail(true, "the email you entered is invalide");
    } else {
    ...
    if (
      dataValueRegister.username !== "" &&
      dataValueRegister.email !== "" &&
      rqExp.test(dataValueRegister.email) &&
      dataValueRegister.password !== "" &&
      lengPassword > 5 &&
      lengPassword < 13
    ) {
      // Empty To Input Form
  ...
  return (
    <>
      ...
      <form onSubmit={handleOnRegister}>
        <h3 className="text-color-black text-2xl capitalize text-center font-semibold">
          create account
        </h3>
        <div className="grid gap-[.5rem] 360:gap-[2rem] mt-[1.5rem] 360:mt-[3rem]">
          <div className="prent_input">
            <p
              className={
                !checkInputUsername.value ? "hidden" : "correction_form"
              }
            >
              {checkInputUsername.text}
            </p>
            <input
              type="text"
              className={
                checkInputUsername.value === true ? "warning-input" : "input"
              }
              placeholder="Input Your Username"
              name="userName"
              value={dataValueRegister.username}
              onChange={input => updateData({ username: input.target.value })}
            />
            <p className="text_input">Username</p>
          </div>
          <div className="prent_input">
            <p
              className={!checkInputEmail.value ? "hidden" : "correction_form"}
            >
            ...
            <input
              type="email"
              className={
                checkInputEmail.value === true ? "warning-input" : "input"
              }
              placeholder="Input Your Email"
              name="email"
              value={dataValueRegister.email}
              onChange={input => updateData({ email: input.target.value })}
            />
          ...
          <div className="prent_input">
            <p
              className={
                !checkInputPassword.value ? "hidden" : "correction_form"
              }
            >
            ...
            <input
              type="password"
              className={
                checkInputPassword.value === true ? "warning-input" : "input"
              }
              placeholder="Input Your Password"
              name="password"
              value={dataValueRegister.password}
              onChange={input => updateData({ password: input.target.value })}
            />
        ...
        <div className="w-full px-6 mt-[2rem] 360:mt-[3rem] mb-[1.5rem]">
          <button
            className="w-full bg-gray-400 transition-all duration-500 hover:bg-color-hover capitalize text-[1.5rem] text-white py-2 rounded-xl"
      ...
      <div className="flex mx-6 text-sm text-gray-500 mb-2 gap-1 items-center">
        ...
        <button
          className="text-base text-color-primary hover:text-color-hover transition-all duration-300"
          onClick={() => submitForm(true, "login")}
        >
          login
        </button>
      </div>
